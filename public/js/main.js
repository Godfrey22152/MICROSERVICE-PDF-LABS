document.addEventListener("DOMContentLoaded", () => {
  ...
  function truncateFilename(fullName) {
    ...
    if (tokens.length > 3) {
      const truncatedName = tokens.slice(0, 3).join('_');
  ...
  function createAudioCard(fileData) {
    ...
    const displayName = truncateFilename(fileData.audioFile);
  ...
  // Truncate filenames on page load
  document.querySelectorAll('.file-name[data-audio-file]').forEach(el => {
    const audioFile = el.dataset.audioFile;
    if (audioFile) {
      el.textContent = truncateFilename(audioFile);
    }
  });
});

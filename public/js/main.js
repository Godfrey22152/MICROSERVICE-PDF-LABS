...
// ---------- Auth / Session ----------
// Accepts a message param, clears token immediately (not inside setTimeout),
...
// Parse a 401 response and call handleAuthError with the right message
...
function checkSession() {
  ...
  if (delay <= 0) {
    // Already expired (e.g. page refreshed after idle)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Truncate any server-rendered filenames
    document.querySelectorAll('.file-name').forEach(updateFilenameDisplay);
});

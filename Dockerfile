...
# =============================================================
# Stage 4: Build Poppler from source (pdftotext only)
#
# Why build from source?
#   Alpine's packaged poppler-utils carries HIGH/CRITICAL CVEs
#   that are patched only in newer upstream releases. Building
#   from source against a pinned upstream tarball gives us a
#   CVE-free pdftotext binary with a minimal dependency footprint.
#
# Why these cmake flags?
#   pdftotext needs: freetype, fontconfig, jpeg, openjpeg, lcms2,
#                    zlib, expat, libpng.
#   It does NOT need: Cairo (rendering), Qt, GLib, NSS, libcurl,
#                     Boost, or TIFF — all disabled to shrink the
#                     attack surface and build time.
#   BUILD_SHARED_LIBS=OFF produces a self-contained static binary
#   so the runtime image needs only the minimal shared libs below.
# =============================================================
...
RUN apk add --no-cache \
    ...
    libpng-dev
WORKDIR /build
# Download in its own layer so re-builds skip the fetch if version is unchanged
...
RUN cmake -B build -G Ninja \
    ...
    # pdftotext and friends
    -DENABLE_UTILS=ON \
    # Unused language/binding layers — all off
    -DENABLE_CPP=OFF \
    -DENABLE_GLIB=OFF \
    -DENABLE_GOBJECT_INTROSPECTION=OFF \
    -DENABLE_QT5=OFF \
    -DENABLE_QT6=OFF \
    # Cairo not required for pdftotext (text extraction only)
    -DWITH_Cairo=OFF \
    # Image/colour libraries used by pdftotext for embedded images in PDFs
    -DENABLE_LIBTIFF=OFF \
    -DENABLE_LIBJPEG=ON \
    -DENABLE_LIBOPENJPEG=openjpeg2 \
    -DENABLE_CMS=lcms2 \
    # Security / network surface — not needed
    -DENABLE_NSS3=OFF \
    -DENABLE_GPGME=OFF \
    -DENABLE_BOOST=OFF \
    -DENABLE_LIBCURL=OFF \
...
# Install only the shared libraries that the static pdftotext binary
# links against at runtime, then create a non-root user — all in one
...
# Copy the compressed Node binary and the pdftotext binary as root
# before dropping privileges (COPY --chown would still work, but
# installing binaries as root then switching user is cleaner).
COPY --from=node-binary /node-bin/node /usr/local/bin/node
COPY --from=poppler-builder /opt/poppler/bin/pdftotext /usr/local/bin/pdftotext

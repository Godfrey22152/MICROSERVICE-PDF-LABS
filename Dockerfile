...
# Install poppler-utils + libstdc++, then immediately upgrade the four
# vulnerable transitive dependencies IN THE SAME LAYER so no intermediate
# layer ever commits the vulnerable versions to the image.
#
# - openjpeg:    CVE-2025-54874  → upgrade to latest patched build
# - tiff:        CVE-2023-52356, CVE-2026-4775 → upgrade to latest patched build
# - sqlite-libs: CVE-2025-70873  → upgrade to latest patched build
# apk upgrade --no-cache <pkg> pulls the highest available version from
# the repo at build time, so this stays effective on future rebuilds too.
RUN apk add --no-cache libstdc++ poppler-utils \
 && apk upgrade --no-cache openjpeg tiff sqlite-libs \

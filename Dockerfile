...
# npm install to resolve all deps including node-fetch and form-data
...
LABEL org.opencontainers.image.title="PDF COMPRESSOR SERVICE" \
      org.opencontainers.image.description="Lightweight PDF compression microservice for PDF Labs (ConvertAPI)" \
...
# Only libstdc++ is needed — no Ghostscript, compression is handled by ConvertAPI
